...
@specialists_router.get('/', dependencies=[Depends(is_authorized)], response_model=SpecialistsCreate)
async def get_orgs() -> list[SpecialistsResponse]:
...
@specialists_router.get('/{id}', dependencies=[Depends(is_authorized)])
...
@specialists_router.put('/', dependencies=[Depends(is_authorized)])
