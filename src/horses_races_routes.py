...
@horses_races_router.get('/', dependencies=[Depends(is_authorized)], response_model=HorsesRacesCreate)
async def get_orgs() -> list[HorsesRacesResponse]:
...
@horses_races_router.get('/{id}', dependencies=[Depends(is_authorized)])
...
@horses_races_router.put('/', dependencies=[Depends(is_authorized)])
async def update_org(payload:HorsesRacesUpdate):
    updated_horses_races = await horses_races_repository.update(payload, id=payload.id, status_code=200)
